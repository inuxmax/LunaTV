...
const VideoCard = forwardRef<VideoCardHandle, VideoCardProps>(function VideoCard(
  ...
  const handleDeleteRecord = useCallback(
    async (e: React.MouseEvent) => {
      ...
      try {
        ...
        throw new Error('Không thể xoá lịch sử xem');
      }
    },
  ...
  const mobileActions = useMemo(() => {
    ...
    if (config.showPlayButton) {
      actions.push({
        id: 'play',
        label: origin === 'live' ? 'Xem trực tiếp' : 'Phát',
      ...
      actions.push({
        id: 'play-new-tab',
        label: origin === 'live' ? 'Mở tab mới để xem' : 'Mở tab mới để phát',
    ...
    if (config.showHeart && from !== 'douban' && actualSource && actualId) {
      ...
      if (from === 'search') {
        ...
        if (searchFavorited !== null) {
          ...
          actions.push({
            id: 'favorite',
            label: currentFavorited ? 'Bỏ yêu thích' : 'Thêm yêu thích',
          ...
          actions.push({
            id: 'favorite-loading',
            label: 'Đang tải trạng thái yêu thích...',
            icon: <Heart size={20} />,
            onClick: () => { }, // 加载中时不响应点击
            disabled: true,
          });
    ...
    if (config.showDoubanLink && actualDoubanId && actualDoubanId !== 0) {
      actions.push({
        id: 'douban',
        label: isBangumi ? 'Chi tiết Bangumi' : 'Chi tiết Douban',
        icon: <Link size={20} />,
        onClick: () => {
          const url = isBangumi
